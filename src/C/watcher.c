...
static bool DoScriptsExist( void );
	static void InitFileInfoBuffer( FileInfo[] );
		static bool DoesShebangExist( FILE* );
		
static int32_t ExecuteWatchLoop( int32_t, char*[] );
	static bool IsPic( char[] );
	static int32_t DelFile( char*, char* );
	static bool IsStopFile( char* );
	static int32_t ExecuteShScript( int32_t, char*[], char* );


ScriptErrCode StartWatching( int arg_count, char** arg_buf )
{
...
static int32_t ExecuteShScript( int32_t dir_count, char* dir_buf[], char *pic_name )
{
	char command_buffer[ COMMAND_BUF_SIZE ];
	memset( command_buffer, 0, COMMAND_BUF_SIZE );
